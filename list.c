...
void * lastList(List * list) {
    if(list->tail == NULL || list->head == NULL || list->current == NULL) return NULL;
...
void pushFront(List * list, void * data) {
    ...
    if(list->head == NULL){
        ...
        NodoNV->next = list->head;
        list->head->prev = NodoNV;
        list->head = NodoNV;
        list->current = NodoNV;
    }
}
...
void pushCurrent(List * list, void * data) {
    ...
    Node* NodoNV = createNode(data);
    NodoNV->next = list->current->next;
    NodoNV->prev = list->current;
    if(list->current->next != NULL){
        list->current->next->prev = NodoNV;
    }else{
        list->tail = NodoNV;
    }
    list->current->next = NodoNV;
}
